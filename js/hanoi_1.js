...
function initializeDrag(disk, e) {
  if (!e) e = event;
  if (stop) {
    alert("不能在点击'stop'按钮之后在继续执行移动盘子的操作！！");
    return;
  }
  indexFr = indexTo;
...
function dropDisk(disk) {
  ...
  if (gameStatus) {
    ...
    if (movectr == minmove) msg = "\n恭喜你!你成功 " + minmove + " 次移动完成。";
    else if (movectr > minmove) msg = "\n你可以做得更好。";
    else msg = "";

    setTimeout(function () { alert("干的漂亮 !!!" + msg); }, 500);
...
function solve(btn) {
  if (btn.value == "Solve") {
    if (movectr > 0 && !gameOver && !stop)
      if (!confirm("当前的游戏将被中止，你想继续吗?")) return;
    btn.value = "Stop";
    initVars();
    stop = false;
    demo = true;
...
function moveDisk() {
  ...
  if (movectr < parseInt(frm.minmove.value)) t = window.setTimeout("moveDisk()", delay);
    ...
    setTimeout(function () { alert(movectr + " 步！你还能够更快完成它吗？"); }, 500);
